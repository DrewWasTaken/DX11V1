...
//ID3D11BlendState* Transparency; //New Transparency State

ID3D11Buffer* meshVertBuff;		//Vertex Buffer
ID3D11Buffer* meshIndexBuff;	//Index Buffer
XMMATRIX meshWorld;				//World Space Matrix
int meshSubsets = 0;			//Subset Number 
std::vector<int> meshSubsetIndexStart;
std::vector<int> meshSubsetTexture;

std::vector<ID3D11ShaderResourceView*> meshSRV; //Pulls Next Global Material (Mesh Shader Resource View)
std::vector<std::wstring> textureNameArray; //Prevents Same Material Loading

//-------------------------------------------------------------
//                     Window Settings
//-------------------------------------------------------------
LPCTSTR WndClassName = "DX11 Platformer";
HWND hwnd = NULL;
HRESULT hr;
...
//-------------------------------------------------------------
//        Create Effects Constant Buffer Structure
//-------------------------------------------------------------
struct cbPerObject
{
	XMMATRIX  WVP;
...
{
	std::wstring matName;	//Material Name
	XMFLOAT4 difColor;		//RGBA Data
	int texArrayIndex;		//MeshSRV Texture Index Value
	bool hasTexture;		//Does Material Use Texture? 
	bool transparent;		//Is Texture Transparent?
};
...
bool LoadObjModel(std::wstring filename,     //.OBJ Filename
	ID3D11Buffer** vertBuff,				 //Mesh Vertex Buffer		(Pointer To Vertices)
	ID3D11Buffer** indexBuff,				 //Mesh Index Buffer		(Pointer To Index List)
	std::vector<int>& subsetIndexStart,      //Stores Offset			(Starting Index Value Of List)
	std::vector<int>& subsetMaterialArray,   //Store Index Value		(Gets material From Mat Vector)
	std::vector<SurfaceMaterial>& material,  //Vector Of Mat Structures (Checks if Mat Exists Before Pushing Back New Material)
	int& subsetCount,						 //Number Of Subsets In Mesh
	bool isRHCoordSys,						 //True If Model Is RH (LH Z Away From Camera | RH Towards)
	bool computeNormals);					 //True Computes Normals, False Uses Files Normals
    ...
    bool computeNormals)
{
    ...
    std::wifstream fileIn(filename.c_str());    //Opens .OBJ File
    std::wstring meshMatLib;                    //String Holds .OBJ Material Library Filename
    ...
    //Set Default For Texture Coords & Normals
    bool hasTexCoord = false;
    bool hasNorm = false;

    //Temp Variables Stored Into Vectors
    std::wstring meshMaterialsTemp;
    int vertPosIndexTemp;
    int vertNormIndexTemp;
    int vertTCIndexTemp;

    wchar_t checkChar;        //Stores One Char At A Time
    std::wstring face;        //String Containing Face Vertices
    int vIndex = 0;           //Vertex Index Count
    ...
    if (fileIn)
    {
        while (fileIn)
        {
            ...
            case 'v':                           //Get Vertex Descriptions
                ...
                {
                    float vz, vy, vx;
                    fileIn >> vx >> vy >> vz;   //Stores Next Three Types

                    if (isRHCoordSys)
                        vertPos.push_back(XMFLOAT3(vx, vy, vz * -1.0f));    //Inverts Z Axis
                    else
                        vertPos.push_back(XMFLOAT3(vx, vy, vz));
                }
                if (checkChar == 't')           //vt - Vert Texture Coords
                {
                    float vtcu, vtcv;
                    fileIn >> vtcu >> vtcv;     //Stores Next Two Types

                    if (isRHCoordSys)    
                        vertTexCoord.push_back(XMFLOAT2(vtcu, 1.0f - vtcv));    //Reverse "v" Axis
                    else
                        vertTexCoord.push_back(XMFLOAT2(vtcu, vtcv));

                    hasTexCoord = true;             //Model Uses Texture Coords
                }
                ...
                if (checkChar == 'n')               //vn - Vert Normal
                {
                    ...
                    if (isRHCoordSys)
                        vertNorm.push_back(XMFLOAT3(vnx, vny, vnz * -1.0f));    //Inverts Z Axis
                    else
            ...
            //New group (Subset)
            case 'g':                                   //g - Defines Group
                checkChar = fileIn.get();
                if (checkChar == ' ')
                {
                    subsetIndexStart.push_back(vIndex); //Start Subset Index
                    subsetCount++;
                }
                break;

            //Get Face Index
            case 'f':                               //f - Defines Faces
                checkChar = fileIn.get();
                if (checkChar == ' ')
                {
                    face = L"";
                    std::wstring VertDef;           //Holds One Vertex Definition A Time
                    triangleCount = 0;

                    checkChar = fileIn.get();
                    while (checkChar != '\n')
                    {
                        face += checkChar;              //Adds Char To Face String
                        checkChar = fileIn.get();       //Gets Next Character
                        if (checkChar == ' ')           //If Its A Space
                            triangleCount++;            //Increase Triangle Count
                    }

                    //Check for space at the end of face string
                    if (face[face.length() - 1] == ' ')
                        triangleCount--;                //Each space adds to triangle count
                    ...
                    if (face.length() > 0)
                    {
                        ...
                        for (int i = 0; i < 3; ++i)     //First three vertices (first triangle)
                        {
                            ...
                            {
                                if (VertDef[j] != '/')    //If there is no divider "/", add a char to vertPart
                                    vertPart += VertDef[j];
                                ...
                                if (VertDef[j] == '/' || j == VertDef.length() - 1)
                                {
                                    ...
                                    if (whichPart == 0)    //If vPos
                                    {
                                        ...
                                        vertPosIndexTemp -= 1;                      //Subtracts one as C++ arrays start with 0, and obj start with 1
                                    ...
                                    else if (whichPart == 1)    //If vTexCoord
                                    {
                                        if (vertPart != L"")    //Check to see if there even is a tex coord
                                        {
                                            ...
                                            vertTCIndexTemp -= 1;                   //subtract one as C++ arrays start with 0, and obj start with 1
                                        }
                                        else    //If there is no tex coord, make a default
                                            vertTCIndexTemp = 0;

                                        //If the cur. char is the second to last in the string
                                        //Must be no normal, so set a default normal
                                        if (j == VertDef.length() - 1)
                                            vertNormIndexTemp = 0;
                            ...
                            if (totalVerts >= 3)                        //Check Number Of Triangles
                            {
                            ...
                            //If vertex is not already in vertex arrays, insert
                            if (!vertAlreadyExists)
                            {
                                vertPosIndex.push_back(vertPosIndexTemp);
                                vertTCIndex.push_back(vertTCIndexTemp);
                                vertNormIndex.push_back(vertNormIndexTemp);
                                totalVerts++;                               //Created a new vertex
                        ...
                        //If more than three vertices in the face definition, convert the face to triangles
                        //Create new triangle for each new vertex in the face, using the very first vertex
    ...
    {
        SwapChain->SetFullscreenState(false, NULL);    //Check not in fullscreen
...
bool InitScene()
{
	...
	hr = D3DX11CreateShaderResourceViewFromFile(d3d11Device, "netherbrick.png",
...
void DrawScene()
{
	...
	float bgColor[4] = { 0.07f, 0.66f, 1.00f, 1.0f };
